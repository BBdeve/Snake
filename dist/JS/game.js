...
export default class Game {
    ...
    renderGameGrid(rows, columns = null) {
        let currentRow = 1;
        this.ctx.beginPath();
...
class Level {
    constructor(ctx) {
        ...
        this.ctx = ctx;
        this.snake = this._initialEntitySet[0];
        this.points = 0;
...
class GameObject {
    constructor({ posX, posY }, { width, height }, parentLevel) {
...
class Snake extends GameObject {
    ...
    grow() {
        let tailPosition = this.tail.length === 0 ? { posX: this.posX, posY: this.posY } : { posX: this.tail[this.tail.length - 1].posX, posY: this.tail[this.tail.length - 1].posY };
        if (this.tail.length === 0) {
        ...
        if (this.parentLevel.points > 0 && this.parentLevel.points % 80 === 0)
            gridSize += 3;
        this.parentLevel.updateLevelScaling();
    }
    tailUpdate(deltaTime) {
        if (this.tail.length === 0)
            return;
        ...
        this.tail.forEach(tailSegment => {
            ...
            if (tailSegment.isMovementLocked)
                tailSegment.isMovementLocked = false;
            else
                tailSegment.move(tailSegment.direction, deltaTime);
        });
        this.watchForCollisions();
    }
    onCollision(gameObject) {
...
class Food extends GameObject {
    ...
    draw(ctx) {
        ctx.beginPath();
        ctx.fillStyle = this.bodyColor;
        ctx.arc((this.posX * this.parentLevel.entityScale.x) + (this.parentLevel.entityScale.x / 2), (this.posY * this.parentLevel.entityScale.y) + (this.parentLevel.entityScale.y / 2), this.width / 3, 0, 2 * Math.PI);
        ctx.fill();
    }
}
function AddControls(snake, controls) {
    window.addEventListener("keydown", (evt) => {
...
function searchValuesInArray(array, elem) {
